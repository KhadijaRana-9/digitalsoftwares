...
import { TICKET_STATUSES, TICKET_PRIORITIES } from '../../lib/constants.js'

const CATEGORIES = ['general', 'billing', 'technical', 'product', 'partner-portal']
...
export default function Support() {
  ...
  const ticketsQ = useSupaQuery(
    ['tickets', user?.id],
    (sb) => sb.from('support_tickets').select('*').eq('partner_id', user.id).order('updated_at', { ascending: false }),
    { enabled: Boolean(user?.id) }
  )
  const messagesQ = useSupaQuery(
    ['ticket_messages', selected?.id],
  ...
  const createMutation = useSupaMutation(
    async (sb, payload) => {
      const { data: ticket, error } = await sb.from('support_tickets').insert({ subject: payload.subject, category: payload.category, priority: payload.priority, partner_id: user.id }).select().single()
      if (error) return { data: null, error }
      const { error: msgError } = await sb.from('support_messages').insert({ ticket_id: ticket.id, sender_id: user.id, sender_role: 'partner', message: payload.message })
      return { data: ticket, error: msgError }
    },
    {
      invalidate: [['tickets', user?.id]],
  ...
  const replyMutation = useSupaMutation(
    (sb, text) => sb.from('support_messages').insert({ ticket_id: selected.id, sender_id: user.id, sender_role: 'partner', message: text }).select().single(),
    { invalidate: [['ticket_messages', selected?.id]], onSuccess: () => setReply(''), onError: (e) => toast.error(e.message) }
  )
  ...
  return (
    <div>
      <PageHeader
        title="Support"
        subtitle="Open a ticket and our team will respond here."
        action={<Button icon={Plus} onClick={() => setModalOpen(true)}>New Ticket</Button>}
      />
      ...
      {ticketsQ.isSuccess && tickets.length === 0 && (
        <EmptyState icon={LifeBuoy} title="No support tickets" description="Need help? Open a ticket and we'll get back to you." action={<Button size="sm" icon={Plus} onClick={() => setModalOpen(true)}>New Ticket</Button>} />
      )}
          ...
          <div className="flex h-[calc(100vh-140px)] flex-col">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto">
              {(messagesQ.data ?? []).map((m) => (
                <div key={m.id} className={`flex ${m.sender_role === 'partner' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${m.sender_role === 'partner' ? 'bg-orange-500 text-white' : 'bg-cream text-ink'}`}>
                    <p>{m.message}</p>
                    <p className={`mt-1 text-[10px] ${m.sender_role === 'partner' ? 'text-orange-100' : 'text-ink-soft'}`}>{timeAgo(m.created_at)}</p>
                  </div>
                </div>
              ))}
            </div>
